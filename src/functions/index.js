...
export async function fHandleSignIn(
  userStorage,
  setDataContext,
  setLoading,
  email,
...
export async function fHandleSignUp(
  setLoading,
  dbSetData,
  handleSignIn,
  name,
  ...
  await auth()
    ...
    .then(async value => {
      await auth()
        .currentUser.updateProfile({displayName: name}) // Atualiza cadastro no banco adicionando displayName
        .then(() => {
          dbSetData(value.user.uid, email, name, pwd); // Grava no banco informações do usuario cadastrado
...
export async function fDbSetData(uid, email, name, pwd) {
  await firestore()
    .collection('users')
    .doc(uid)
    .set({
